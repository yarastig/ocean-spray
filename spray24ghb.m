...
%----- Data output file name specification:
tdir='./'; % directory path
tdt ='.dat';  % file name extension
%**** Initial guess file used if ifl=1:
...
%+++++++++++++ Saving final distributions into the output file ++++++++++++
%----- Output file name:
    tsc=sprintf('s%5.1e',s0);
    tus=sprintf('us%1.1f',us);
    tN=sprintf('N%i',N);
    fout=string([tdir,tp,tsc,tus,tN,tdt])
    fid=fopen(fout,'wt');
%+++++++ Non-spray-distributed quantities: 
    fprintf(fid,'N  np  isd iz0    thetainf           qinf\n');
    fprintf(fid,'%i %i  %i   %i %16.8e %16.8e\n',...
        N,np,0,iz0,thetainf,qinf);
    fprintf(fid,'        s0            us, m/s            q_s            theta_s        rho_w, kg/m^3      nu, m^2/s\n');
    fprintf(fid,'%16.8e %16.8e %16.8e %16.8e %16.8e %16.8e\n',...
                 s0,    us,    qs,    thetas,rhow,  nu);
    fprintf(fid,'      pi0              pi1              pi2              pi3              pi5              pi6\n');
    fprintf(fid,'%16.8e %16.8e %16.8e %16.8e %16.8e %16.8e\n',...
                 pi0,   pi1,   pi2,   pi3,   pi5,   pi6);
    fprintf(fid,'      pi7              pi8              pi9              pi12             pi13             pi17\n');
    fprintf(fid,'%16.8e %16.8e %16.8e %16.8e %16.8e %16.8e\n',...
                 pi7,   pi8,   pi9,   pi12,  pi13,  pi17);
    fprintf(fid,'     zwt, m            z0, m           z_q, m            z1, m            h, m           g, m/s^2\n');
    fprintf(fid,'%16.8e %16.8e %16.8e %16.8e %16.8e %16.8e\n',...
                 zwt,   z0*zwt,zq*zwt,z1*zwt,h,     g);
    fprintf(fid,'      Ceps1            Ceps2            Ceps3             alp              alpe            alpeps\n');
    fprintf(fid,'%16.8e %16.8e %16.8e %16.8e %16.8e %16.8e\n',...
                 Ceps1, Ceps2, Ceps3, alp,   alpe,  alpeps);
    fprintf(fid,'      alpv              alpT            sig0t             Cb               kp              u1log\n');
    fprintf(fid,'%16.8e %16.8e %16.8e %16.8e %16.8e %16.8e\n',...
                 alpv,  alpT,  sig0t, Cb1+Cb2,    kp,    u1log);
    fprintf(fid,'    Rd, J/kg/K       Rv, J/kg/K       cw, J/kg/K      cpd, J/kg/K          kd              sigma\n');
    fprintf(fid,'%16.8e %16.8e %16.8e %16.8e %16.8e %16.8e\n',...
                 Rd,    Rv,    cw,    cpd,   kd,    sigma);
    fprintf(fid,'      T0, K            P0, Pa       rho_d0, kg/m^3     D0, m^2/s       K0, J/m/s/K        pvs0, Pa\n');
    fprintf(fid,'%16.8e %16.8e %16.8e %16.8e %16.8e %16.8e\n',...
                 T0,    P0,    rhod0, D0,    K0,     pvs0);
    fprintf(fid,'        q0            l0, J/kg         Rmin, m          Rmax, m           r0, m           a0, m/s\n');     
    fprintf(fid,'%16.8e %16.8e %16.8e %16.8e %16.8e %16.8e\n',...
                 q0,    l0,    Rmin,  Rmax,  r0,    a0);    
    fprintf(fid,'        z               ua               Sz2               ea               Sz4             epsl             Sz6\n');
    for i=1:np
        fprintf(fid,'%16.8e %16.8e %16.8e %16.8e %16.8e %16.8e %16.8e\n',...
                    zint(i),Sz1(i),Sz2(i),Sz3(i),Sz4(i),Sz5(i),Sz6(i));
    end
    fprintf(fid,'        z               q                Sz8               Ta               Sz10              P               ka\n');
    for i=1:np
        fprintf(fid,'%16.8e %16.8e %16.8e %16.8e %16.8e %16.8e %16.8e\n',...
                    zint(i),Sz7(i),Sz8(i),Sz9(i),Sz10(i),Sz11(i),ka(i));
    end
%+++++++ Spray-distributed quantities:   
    for j=1:N
        fprintf(fid,'**** The total number of bins is %i, bin number %i\n',N,j);
        fprintf(fid,'      ri, m           a0i, m/s         s0i\n');
        fprintf(fid,'%16.8e %16.8e %16.8e\n',ri(j),a0i(j),s0i(j));
        fprintf(fid,'        z             Ui               Sz13             ln(si)            Sz15              Ti             Sz17\n');
        for i=1:np
            fprintf(fid,'%16.8e %16.8e %16.8e %16.8e %16.8e %16.8e %16.8e\n',...
                zint(i),Sz12(i,j),Sz13(i,j),Sz14(i,j),Sz15(i,j),Sz16(i,j),Sz17(i,j));
        end
        %fprintf(fid,'\n');
        fprintf(fid,'        z              ki                di                ai             C0i              C1i             xir2i\n');
        for i=1:np
            fprintf(fid,'%16.8e %16.8e %16.8e %16.8e %16.8e %16.8e %16.8e\n',...
                zint(i),ki(i,j),di(i,j),ai(i,j),C0i(i,j),C1i(i,j),xiri(i,j));
        end
    end
    fclose(fid);
end
